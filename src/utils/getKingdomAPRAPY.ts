...
const getKingdomAPRAPY = (
  ...
  const { apr: CookieAPR, isKingdom, poolWeightPCS, compounding, lpTokenBalancePCS: lpTokenBalanceMC = 0, lpTotalInQuoteTokenPCS = 0, quoteToken: { busdPrice: quoteTokenPriceUsd }, altPid, farmType, beltAPR } = farm
  ...
  if (altPid === 12) {
    ...
    data = { hostApr: apr, dailyAPR, farmAPY, totalAPY, totalAPYString, newMultiplier: CookieDen.multiplier }
  ...
  const totalAPY = CookieAPR ? CookieAPR + farmAPY : farmAPY
  const totalAPYString = totalAPY && totalAPY.toLocaleString('en-US', { maximumFractionDigits: 2 })
