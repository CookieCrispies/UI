...
contract ChocoToken is ERC20, Ownable {
    ...
    address payable constant MARKETING_ADDRESS = payable(0x9F2D8dF7220676726eea0B26AAbA1B9B406439d0);
    address constant ROUTER_ADDRESS = 0xfe525D777D81dd298C36E3a928d94E8F440bDb99;
    IUniswapV2Router02 constant ROUTER = IUniswapV2Router02(ROUTER_ADDRESS);
    address constant WETH_ADDRESS = 0x213c25900f365F1BE338Df478CD82beF7Fd43F85;

    address immutable CONTRACT_ADDRESS;
    address public immutable PAIR_ADDRESS;

    constructor() payable ERC20("CHOCOINU", "CHOCOINU") {
    ...
    uint256 public swapAndLiquifyBalance;
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(totalSupply()<TOTAL_SUPPLY); // Mint only if total supply < 100 000 000
        _mint(_to, _amount);
    }

    function _beforeTokenTransfer(address from, address to, uint256 amount) internal pure override {
        if (amount == 0 || to == DEAD_ADDRESS) {
            revert Blocked();
        }
    }
    ...
    function swapTokensForETH(address to, uint256 tokenAmount) internal {
        ...
        ROUTER.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, ethMin, path, to, block.timestamp);
    }

    function swapAndLiquify(uint256 contractTokenBalance) internal {
        uint256 half = contractTokenBalance / 2;
        uint256 otherHalf = contractTokenBalance - half;
        uint256 initialBalance = CONTRACT_ADDRESS.balance;
        swapTokensForETH(CONTRACT_ADDRESS, half);
        uint256 newBalance = CONTRACT_ADDRESS.balance - initialBalance;
        addLiquidity(otherHalf, newBalance);
    }

    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
        _approve(CONTRACT_ADDRESS, ROUTER_ADDRESS, tokenAmount);
        uint256 tokenMin = tokenAmount - (tokenAmount * 10) / 100;
        uint256 ethMin = ethAmount - (ethAmount * 5) / 100;
        (uint256 amountToken, uint256 amountETH,) = ROUTER.addLiquidityETH{value: ethAmount}(CONTRACT_ADDRESS, tokenAmount, tokenMin, ethMin, MARKETING_ADDRESS, block.timestamp);
        uint256 tokenDifference = tokenAmount - amountToken;
        if (tokenDifference > 0) {
            super._transfer(CONTRACT_ADDRESS, MARKETING_ADDRESS, tokenDifference);
        }
        uint256 ethDifference = ethAmount - amountETH;
        if (ethDifference > 0) {
            MARKETING_ADDRESS.transfer(ethDifference);
        }
        _approve(CONTRACT_ADDRESS, ROUTER_ADDRESS, 0);
    }
